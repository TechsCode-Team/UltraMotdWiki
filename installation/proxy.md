...
`Note:` *You will need a MySQL database to be able to use Ultra Motd in proxy mode. Depending on your host, the process can vary, and will not be mentionned nor explained here.*
<br>
...
## Connecting to MySQL
As mentionned above, a MySQL database is needed to sync Ultra Motd accross servers. To start your syncing process, connect to your server in-game, and open the plugin's GUI by executing the command `/umotd` in chat.
<br>
...
`/umotd >> Settings >> MySQL Database >> Setup MySQL >> Credentials`
<br>
